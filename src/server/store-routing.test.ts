...
describe('FileStore routing compatibility', () => {
  it('round-trips optional Codex route fields and treats absent agent as legacy Devin', () => {
    ...
    const created = createCard(state, wsId, { title: 'route', description: 'task', cwd: '/tmp', skipApprovals: true });
    ...
    expect(card).toMatchObject({ skipApprovals: true, agent: 'codex', modelId: 'capable', route: { model: 'openai/gpt-6-astra' } });
